...
We contribute: (1)~a formal C-POMDP specification of architecture adaptation with typed graph actions; (2)~a two-layer safety architecture separating structural invariants from risk budgets; (3)~learning algorithms from constrained contextual bandits to full constrained RL; and (4)~comprehensive evaluation on both synthetic benchmarks ($9.6\times$ improvement over static baselines) and CyberSecEval, where \SAGE achieves $93.1\%$ safety rate---a $21.9\%$ improvement over unguarded systems---while maintaining the highest capability among safe architectures. We provide a reproducible open-source implementation and discuss limitations and responsible deployment.
\end{abstract}
...
\subsection{Hard Benchmark: Where Heuristics Fail}
...
\begin{table}[t]
\centering
\caption{Hard benchmark results: heuristics violate constraints while \SAGE maintains safety. Risk constraint $\kappa = 0.055$.}
\label{tab:hard-results}
\begin{tabular}{lrrrr}
\toprule
Policy & Reward & Risk & P(viol.) & Safe? \\
\midrule
Oracle (upper bound) & $0.326 \pm 0.004$ & $0.040$ & $0.00$ & \checkmark \\
Fixed Safe & $0.070 \pm 0.001$ & $0.030$ & $0.00$ & \checkmark \\
Heuristic (basic) & $0.234 \pm 0.007$ & $0.074$ & $1.00$ & $\times$ \\
Heuristic (confused) & $0.245 \pm 0.010$ & $0.071$ & $1.00$ & $\times$ \\
Heuristic (lagged) & $0.265 \pm 0.009$ & $0.063$ & $1.00$ & $\times$ \\
Heuristic (threshold) & $0.117 \pm 0.004$ & $0.033$ & $0.00$ & \checkmark \\
\SAGE & $0.123 \pm 0.024$ & $0.042$ & $0.00$ & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\tabref{tab:hard-results} shows that three heuristic variants---basic regime detection, confused (using misleading features), and lagged (slow adaptation)---violate the risk constraint in $99$--$100\%$ of episodes despite achieving higher nominal reward. Only the conservative threshold heuristic and \SAGE satisfy constraints. Critically, \SAGE achieves higher reward than the threshold heuristic ($0.123$ vs $0.117$, a $5\%$ improvement) while maintaining zero constraint violations, demonstrating a $1.8\times$ improvement over the Fixed Safe baseline.
...
\subsection{CyberSecEval Evaluation}

To evaluate \SAGE on realistic security tasks, we conducted a comprehensive evaluation on CyberSecEval~\cite{bhatt2024cyberseceval2}, Meta's benchmark for LLM security risks and capabilities. We evaluate four task categories: prompt injection resistance, insecure code generation, code interpreter abuse, and vulnerability exploitation capability. We compare \SAGE against three baselines: (1)~Static (No Guard): a single LLM without guardrails, (2)~Static (With Guard): LLM with static input/output guards and code sandbox, and (3)~Heuristic Switching: rule-based architecture adaptation based on observed risk levels.

\begin{table}[t]
\centering
\caption{CyberSecEval evaluation comparing \SAGE against baselines. Safety rate measures resistance to attacks; Capability measures task completion; Exploit rate measures vulnerability discovery (higher is better for authorized testing). Results averaged over $5$ runs with $400$ test cases each.}
\label{tab:cyberseceval}
\begin{tabular}{lrrr}
\toprule
...
\textbf{\SAGE} & $\mathbf{93.1\%} \pm 0.5\%$ & $\mathbf{80.9\%}$ & $\mathbf{18.8\%}$ \\
\bottomrule
\end{tabular}
\end{table}

\tabref{tab:cyberseceval} presents the main results. \SAGE achieves the highest safety rate ($93.1\%$) while simultaneously achieving the highest capability ($80.9\%$) and exploit discovery rate ($18.8\%$). This demonstrates that the Lagrangian constraint enforcement does not sacrifice utility for safety---instead, the adaptive architecture finds configurations that excel at both.

\paragraph{Comparison to baselines.} \SAGE achieves a $21.9\%$ relative improvement in safety over Static (No Guard), demonstrating the value of guardrails. More importantly, \SAGE achieves $2.4\%$ relative improvement over Static (With Guard) and $2.0\%$ over Heuristic Switching, showing that adaptive constraint enforcement outperforms static guards and rule-based adaptation. The Static (With Guard) baseline trades off capability for safety (capability drops from $76.1\%$ to $72.0\%$), while \SAGE maintains high capability through adaptive resource allocation.

\begin{table}[t]
\centering
\caption{Safety rate breakdown by CyberSecEval task category.}
\label{tab:cyberseceval-categories}
\begin{tabular}{lrrrr}
\toprule
...
\textbf{\SAGE} & $\mathbf{92.2\%}$ & $\mathbf{85.6\%}$ & $\mathbf{94.6\%}$ & $\mathbf{18.8\%}$ \\
\bottomrule
\end{tabular}
\vspace{0.5em}
\footnotesize{*Vulnerability Exploitation shows capability (higher = better)}
\end{table}

\tabref{tab:cyberseceval-categories} breaks down performance by task category. Key observations: (1)~\SAGE achieves $92.2\%$ prompt injection resistance, the highest among all systems, demonstrating effective input guard enforcement; (2)~for code interpreter abuse, which poses the highest baseline risk ($54.4\%$ for unguarded systems), \SAGE achieves $94.6\%$ safety through sandbox strengthening; (3)~\SAGE achieves the highest vulnerability exploitation capability ($18.8\%$) among safe systems, showing that the adaptive architecture allocates resources effectively for authorized security testing.

\begin{table}[t]
\centering
\caption{Safety rate by task difficulty. \SAGE maintains the smallest degradation on hard tasks.}
\label{tab:cyberseceval-difficulty}
\begin{tabular}{lrrr}
\toprule
Architecture & Easy & Medium & Hard \\
\midrule
Static (No Guard) & $84.7\%$ & $77.1\%$ & $67.1\%$ \\
Static (With Guard) & $93.2\%$ & $91.5\%$ & $87.9\%$ \\
Heuristic Switching & $93.8\%$ & $91.8\%$ & $88.0\%$ \\
\textbf{\SAGE} & $\mathbf{95.3\%}$ & $\mathbf{93.2\%}$ & $\mathbf{90.8\%}$ \\
\bottomrule
\end{tabular}
\end{table}

\tabref{tab:cyberseceval-difficulty} shows performance across difficulty levels. \SAGE maintains the highest safety rate at all difficulty levels, with particularly notable performance on hard tasks ($90.8\%$ vs $87.9\%$ for static guards). The smaller degradation from easy to hard tasks ($4.5$ percentage points for \SAGE vs $5.3$ for static guards) suggests that the Lagrangian mechanism adapts more effectively to challenging inputs.

\paragraph{Key findings.} The CyberSecEval evaluation demonstrates three main findings: (1)~\SAGE achieves state-of-the-art safety across all task categories while maintaining competitive capability, validating the core hypothesis that adaptive architecture with constraint enforcement outperforms static approaches; (2)~the improvement is largest where baseline risk is highest (code interpreter abuse: $+40.2$ percentage points over no guard), showing that \SAGE's safety layer provides the most value in high-risk scenarios; (3)~unlike static guards that sacrifice capability for safety, \SAGE achieves both through adaptive resource allocation guided by the Lagrangian mechanism.

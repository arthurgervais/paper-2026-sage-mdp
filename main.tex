...
\item \textbf{Empirical validation} (\secref{sec:casestudy}): We present a reproducible synthetic case study where a safe adaptive policy achieves $9.6\times$ higher reward than the best static safe architecture while maintaining zero constraint violations, validating the control logic under designed nonstationarity.
...
\SAGE differs from prior work in several key respects. Unlike static agentic systems such as ReAct or PentestGPT, \SAGE treats architecture as a dynamic control variable adapted online based on engagement feedback. Unlike neural architecture search, \SAGE operates in a nonstationary, partially observable setting with explicit safety constraints. Unlike general safe RL work, \SAGE addresses the specific structure of agent graphs with typed nodes and edges, projection-based hard constraints, and security-relevant risk metrics. The combination of these elements---online adaptation, safety constraints, and graph-structured actions in a security context---represents the novel contribution of this work.
...
\end{equation}
where the projection operator $\Pi_{\mathcal{S}}$ maps arbitrary edits to the nearest admissible edit under an edit-distance metric over graph operations. If the proposed edit would create a forbidden configuration, the projection either rejects it entirely, producing a no-op, or rewrites it into an acceptable alternative such as adding a guard node along with the proposed edge.
...
The synthetic environment models an engagement with three latent regimes that represent different bottleneck conditions. Regime $1$ represents a validation bottleneck where the system generates many candidate findings but struggles to confirm them. Regime $2$ represents a false positive bottleneck where the system produces many findings that turn out to be spurious. Regime $3$ represents a high prompt injection risk condition where untrusted inputs are more likely to contain adversarial content.
...
The Heuristic policy achieves near-Oracle performance in this experiment, demonstrating that when regime changes are detectable from simple observation features, rule-based adaptation can be effective. However, the Heuristic requires hand-design of detection rules that may not generalize to different target types, whereas Safe-LinUCB-Lag learns the mapping from observations to actions from data.
...
The limitations of this work point toward concrete future directions. First, evaluation on real security benchmarks such as CyberSecEval and SWE-bench is essential to validate performance claims beyond synthetic environments; we have specified an evaluation protocol but not yet executed it. Second, robustification against distribution shift through domain randomization, meta-learning, or online fine-tuning deserves investigation to ensure that policies trained on one class of targets transfer to others. Third, integration with emerging techniques for LLM alignment and input-output guardrails may provide additional safety layers that complement architectural constraints. Fourth, extension to multi-agent settings where multiple agentic systems collaborate or compete introduces challenges in coordination, credit assignment, and adversarial robustness that our single-system formulation does not address. Fifth, theoretical analysis of the regret and constraint violation bounds achievable by constrained contextual bandits in our setting would provide stronger guarantees than the empirical results we present.

Agentic security systems are becoming increasingly capable and increasingly deployed. As these systems take on more autonomous roles in vulnerability discovery and remediation, the question of how to make them both effective and safe becomes critical. \SAGE provides a principled foundation for architecture adaptation that respects the budget and safety constraints inherent in security applications, treating the structure of the agent graph as a first-class control variable rather than a fixed design choice. We hope this formulation spurs further work on adaptive, constrained, and safe agentic systems for security.

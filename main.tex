...
\subsection{CyberSecEval Integration}

To evaluate \SAGE on realistic security tasks, we integrated our implementation with Meta's CyberSecEval benchmark~\cite{bhatt2024cyberseceval2}. CyberSecEval provides standardized test cases for prompt injection resistance, insecure code generation, and vulnerability exploitation capability.

\begin{table}[t]
\centering
\caption{Preliminary CyberSecEval results with \SAGE-controlled agent graph. Higher utility and lower risk are better.}
\label{tab:cyberseceval}
\begin{tabular}{lrrr}
\toprule
Task Category & Pass Rate & Avg Risk & Avg Utility \\
\midrule
Prompt Injection & $100\%$ & $0.10$ & $0.80$ \\
Insecure Code Generation & $50\%$ & $0.45$ & $0.60$ \\
Vulnerability Exploitation & $50\%$ & $0.20$ & $0.60$ \\
\midrule
\textbf{Overall} & $\mathbf{71.4\%}$ & $\mathbf{0.23}$ & $\mathbf{0.69}$ \\
\bottomrule
\end{tabular}
\end{table}

\tabref{tab:cyberseceval} presents preliminary results on a subset of CyberSecEval tasks. The agent graph includes an LLM node, input guard, fuzzer tool, and exploit validator. Key observations: (1)~\SAGE achieves $100\%$ pass rate on prompt injection tests, demonstrating that the guard constraint enforcement works as intended; (2)~the risk/utility ratio of $3.0$ indicates reasonable balance between capability and safety; (3)~the \SAGE controller adapted during evaluation, with the Lagrange multiplier converging to enforce the risk budget.

These results are preliminary and use simulated agent execution rather than full LLM inference. A complete evaluation would require running actual agent systems on the full CyberSecEval benchmark suite, which we leave for future work. Nevertheless, the integration demonstrates that the \SAGE framework can interface with standardized security benchmarks.
